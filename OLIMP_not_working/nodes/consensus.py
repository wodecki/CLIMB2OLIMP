...
def consensus(state: DocumentState) -> DocumentState:
    ...
    """
    print("🔍 consensus node called!")
    print(f"DEBUG: consensus - Node execution started at: {__import__('time').strftime('%Y-%m-%d %H:%M:%S')}")
    import time
...
"""
        ...
        node_end = time.time()
        print(f"DEBUG: consensus - Node completed in {node_end - node_start:.2f} seconds")
        print(f"DEBUG: consensus - Generated {len(consensus_recommendation)} character report")
        
        return state
        
    except Exception as e:
        node_end = time.time()
        print(f"DEBUG: consensus - ERROR after {node_end - node_start:.2f} seconds: {e}")
