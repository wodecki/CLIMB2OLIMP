...
def recommend_A(state: DocumentState) -> DocumentState:
    ...
    print("🔍 recommend_A node called!")
    print(f"DEBUG: recommend_A - Node execution started at: {__import__('time').strftime('%Y-%m-%d %H:%M:%S')}")
    import time
    node_start = time.time()
    
    result = generate_recommendation_for_branch(state, "A", "openai")
    
    node_end = time.time()
    print(f"DEBUG: recommend_A - Node completed in {node_end - node_start:.2f} seconds")
    return result

def recommend_B(state: DocumentState) -> DocumentState:
    """Branch B: Claude recommendations"""
    print("🔍 recommend_B node called!")
    print(f"DEBUG: recommend_B - Node execution started at: {__import__('time').strftime('%Y-%m-%d %H:%M:%S')}")
    import time
    node_start = time.time()
    
    result = generate_recommendation_for_branch(state, "B", "anthropic")
    
    node_end = time.time()
    print(f"DEBUG: recommend_B - Node completed in {node_end - node_start:.2f} seconds")
    return result

def recommend_C(state: DocumentState) -> DocumentState:
    """Branch C: Gemini recommendations"""
    print("🔍 recommend_C node called!")
    print(f"DEBUG: recommend_C - Node execution started at: {__import__('time').strftime('%Y-%m-%d %H:%M:%S')}")
    import time
    node_start = time.time()
    
    result = generate_recommendation_for_branch(state, "C", "gemini")
    
    node_end = time.time()
    print(f"DEBUG: recommend_C - Node completed in {node_end - node_start:.2f} seconds")
    return result